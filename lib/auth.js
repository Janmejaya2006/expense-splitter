...
export function isEmailVerificationRequired() {
  const emailDeliveryReady = isAuthEmailDeliveryConfigured();
  const defaultEnabled = process.env.NODE_ENV === "production" && emailDeliveryReady;
  const requested = envToggleOrDefault(process.env.AUTH_REQUIRE_EMAIL_VERIFICATION, defaultEnabled);
  return Boolean(requested && emailDeliveryReady);
}
...
function linkMembersToUserByEmail(draft, userId, email) {
  const normalizedId = Number(userId || 0);
  const normalizedEmail = normalizeEmail(email);
  if (normalizedId <= 0 || !normalizedEmail) return;

  for (const member of draft.members || []) {
    const memberEmail = normalizeEmail(member.email);
    if (memberEmail && memberEmail === normalizedEmail) {
      member.userId = normalizedId;
    }
  }
}
...
export async function authenticateUser(email, password) {
  ...
  await updateDB((draft) => {
    ...
    linkMembersToUserByEmail(draft, match.id, match.email);
    loggedInUser = { ...match };
    return draft;
  });
...
export async function verifyLoginOtpChallenge(challengeToken, otpCode) {
  ...
  await updateDB((draft) => {
    ...
    linkMembersToUserByEmail(draft, user.id, user.email);
    verifiedUser = { ...user };
...
export function isOtpLoginEnabled() {
  const emailDeliveryReady = isAuthEmailDeliveryConfigured();
  const defaultEnabled = process.env.NODE_ENV === "production" && emailDeliveryReady;
  const requested = envToggleOrDefault(process.env.AUTH_2FA_ENABLED, defaultEnabled);
  return Boolean(requested && emailDeliveryReady);
}
