...
const PROOF_ROOT = path.join(process.cwd(), "data", "proofs");
...
async function storeBinaryProof({ groupId, fileName, mimeType, base64, prefix = "proof" }) {
  ...
  const relativePath = path.relative(path.join(process.cwd(), "data"), absolutePath).replaceAll(path.sep, "/");
  return {
...
export function resolveStoredProofPath(relativePath) {
  ...
  const absolutePath = path.join(process.cwd(), "data", safeRelative);
