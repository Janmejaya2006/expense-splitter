...
import { sendMonthlySummaryEmail, sendWeeklySummaryEmail } from "@/lib/notifications";
...
function previousWeekRangeUtc(date) {
  const currentWeekStart = weekStartUtc(date);
  const previousWeekStart = new Date(currentWeekStart);
  previousWeekStart.setUTCDate(previousWeekStart.getUTCDate() - 7);
  const previousWeekEnd = new Date(currentWeekStart);
  previousWeekEnd.setUTCMilliseconds(previousWeekEnd.getUTCMilliseconds() - 1);

  return {
    key: previousWeekStart.toISOString().slice(0, 10),
...
export async function runMaintenanceJobs({ notificationLimit = 25 } = {}) {
  ...
  let weeklySummaryEmailsSent = 0;
  let weeklySummaryGroupsProcessed = 0;
  let weeklySummaryGroupsSkipped = 0;
  ...
  if (nowDate.getUTCDay() === 1) {
    const summaryDb = await readDB();
    const targetRange = previousWeekRangeUtc(nowDate);
    const compareRange = previousWeekRangeUtc(targetRange.startDate);
    const weeklyGroupsToMark = [];
    const usersByEmail = new Map(
      (summaryDb.users || []).map((user) => [String(user.email || "").trim().toLowerCase(), user])
    );
    ...
    for (const group of summaryDb.groups || []) {
      if (String(group.lastWeeklySummaryWeek || "") === targetRange.key) {
        weeklySummaryGroupsSkipped += 1;
        continue;
      }

      const groupId = Number(group.id);
      const members = (summaryDb.members || []).filter((item) => Number(item.groupId) === groupId);
      const weeklyExpenses = (summaryDb.expenses || []).filter(
        (item) => Number(item.groupId) === groupId && inRange(item.expenseDate || item.createdAt, targetRange)
      );
      const weeklyPayments = (summaryDb.settlementPayments || []).filter(
        (item) => Number(item.groupId) === groupId && inRange(item.createdAt, targetRange)
      );
      const previousExpenses = (summaryDb.expenses || []).filter(
        (item) => Number(item.groupId) === groupId && inRange(item.expenseDate || item.createdAt, compareRange)
      );
      const previousPayments = (summaryDb.settlementPayments || []).filter(
        (item) => Number(item.groupId) === groupId && inRange(item.createdAt, compareRange)
      );

      const summary = buildGroupSummary(group, members, weeklyExpenses, weeklyPayments);
      const previousSummary = buildGroupSummary(group, members, previousExpenses, previousPayments);
      const topCategory = summary.categoryBreakdown?.[0] || null;
      const insights = [];
      const diffLine = monthDiffInsight(summary.totalSpent, previousSummary.totalSpent);
        ...
        new Set(
          members
            ...
            .filter((email) => {
              ...
              return prefs.email !== false && prefs.weeklySummary !== false;
            })
        )
      );
      ...
      for (const email of recipients) {
        try {
          ...
          await sendWeeklySummaryEmail({
            ...
            recipientName: member?.name || "",
            groupName: group.name,
            periodLabel: weekLabel(targetRange),
            currency: group.currency || "INR",
            totalSpent: summary.totalSpent,
            expenseCount: summary.expenseCount,
            settledAmount: summary.settledAmount,
          ...
          logError("jobs.weekly_summary_email_failed", {
            groupId,
            email,
            week: targetRange.key,
            error: error instanceof Error ? error.message : "Unknown error",
          });
        }
      }
      ...
      if (sentForGroup > 0 || recipients.length === 0) {
        weeklyGroupsToMark.push(groupId);
      }
      weeklySummaryEmailsSent += sentForGroup;
      weeklySummaryGroupsProcessed += 1;
    }

    if (weeklyGroupsToMark.length) {
      await updateDB((draft) => {
        for (const group of draft.groups || []) {
          if (!weeklyGroupsToMark.includes(Number(group.id))) continue;
          group.lastWeeklySummaryWeek = targetRange.key;
          appendActivity(draft, {
            groupId: Number(group.id),
            type: "weekly_summary_sent",
            message: `Weekly summary email processed for week starting ${targetRange.key}.`,
            createdByUserId: null,
          });
        }
        return draft;
  ...
  const result = {
    ...
    weeklySummaryEmailsSent,
    weeklySummaryGroupsProcessed,
    weeklySummaryGroupsSkipped,
