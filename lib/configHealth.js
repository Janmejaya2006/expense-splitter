...
export function getConfigHealth() {
  ...
  const resendKey = asString(process.env.RESEND_API_KEY);
  const emailFrom = asString(process.env.EMAIL_FROM);
  const resendReady = resendKey.startsWith("re_") && !isPlaceholder(resendKey) && Boolean(emailFrom);
  checks.push(
    buildCheck({
      key: "email_delivery",
      label: "Email Delivery",
      status: resendReady ? "ok" : "warn",
      message: resendReady
        ? `Ready (${masked(emailFrom)})`
        : "Set RESEND_API_KEY + EMAIL_FROM to enable invite and reset emails.",
      required: false,
      meta: null,
    })
  );

  const twoFactorRaw = asString(process.env.AUTH_2FA_ENABLED || "true").toLowerCase();
  const twoFactorEnabled = !["0", "false", "off", "no"].includes(twoFactorRaw);
  checks.push(
    buildCheck({
      key: "auth_2fa",
      label: "Email OTP Login (2FA)",
      status: !twoFactorEnabled || resendReady ? "ok" : "warn",
      message: twoFactorEnabled
        ? resendReady
          ? "Enabled and email delivery is ready."
          : "Enabled but email provider is not configured yet."
        : "Disabled (AUTH_2FA_ENABLED=false).",
      required: false,
    })
  );

  const emailVerificationRaw = asString(process.env.AUTH_REQUIRE_EMAIL_VERIFICATION || "true").toLowerCase();
  const emailVerificationEnabled = !["0", "false", "off", "no"].includes(emailVerificationRaw);
  checks.push(
    buildCheck({
      key: "auth_email_verification",
      label: "Email Verification",
      status: emailVerificationEnabled ? (resendReady ? "ok" : "warn") : "warn",
