...
export async function sendWeeklySummaryEmail({
  ...
  lines.push(`Hi ${recipientName || "there"},`);
  lines.push("");
  lines.push(`Here is your weekly summary for "${groupName}" (${periodLabel}).`);
  lines.push("");
  lines.push(`Total spent: ${formatCurrency(totalSpent, currency)}`);
  lines.push(`Expenses recorded: ${Number(expenseCount || 0)}`);
  lines.push(`Settled amount: ${formatCurrency(settledAmount, currency)}`);
  ...
  if (topCategories.length) {
    lines.push("Top categories:");
    for (const category of topCategories.slice(0, 4)) {
      lines.push(`  - ${category.category}: ${formatCurrency(category.amount, currency)}`);
    }
    lines.push("");
  }
  ...
  if (insightLines.length) {
    lines.push("Insights:");
    for (const line of insightLines.slice(0, 4)) {
      lines.push(`  - ${line}`);
    }
    lines.push("");
  }
  ...
  return sendGmailEmail({
    toEmail,
    subject: `Weekly summary • ${groupName} • ${periodLabel}`,
    text: lines.join("\n"),
    errorMessage: "Failed to send weekly summary email",
    successMessage: "Weekly summary email sent successfully",
  });
}
