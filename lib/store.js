...
import { BUNDLED_DB_JSON_PATH, DATA_DIR, DB_JSON_PATH, DB_SQLITE_PATH } from "./dataPaths.js";
...
async function readJsonStateFromPath(filePath) {
  try {
...
async function readJsonFileFallback() {
  const primary = await readJsonStateFromPath(DB_JSON_PATH);
  if (primary && typeof primary === "object") return primary;

  const resolvedPrimaryPath = path.resolve(DB_JSON_PATH);
  const resolvedBundledPath = path.resolve(BUNDLED_DB_JSON_PATH);
  if (resolvedPrimaryPath !== resolvedBundledPath) {
    const bundled = await readJsonStateFromPath(BUNDLED_DB_JSON_PATH);
